...
def analyze_chess_move():
    ...
    # Generate sample chess boards for display with next moves and analysis
    sample_boards = {}
    sample_positions = [
        ('italian_game', 'r1bqkbnr/pppp1ppp/2n5/2b1p3/2B1P3/5N2/PPPP1PPP/RNBQK2R w KQkq - 4 4', 'Italian Game (4 moves)', '1.e4 e5 2.Nf3 Nc6 3.Bc4 Bc5', 'd3'),
        ('french_defense', 'rnbqkbnr/ppp2ppp/4p3/3p4/3PP3/8/PPP2PPP/RNBQKBNR w KQkq d6 0 3', 'French Defense (3 moves)', '1.e4 e6 2.d4 d5', 'exd5'),
        ('queens_gambit', 'rnbqkbnr/ppp1pppp/8/3p4/2PP4/8/PP2PPPP/RNBQKBNR b KQkq c3 0 2', "Queen's Gambit (2 moves)", '1.d4 d5 2.c4', 'dxc4'),
        ('caro_kann', 'rn1qkbnr/pp2pppp/2p5/3p4/3PP1b1/5N2/PPP2PPP/RNBQKB1R w KQkq - 1 4', 'Caro-Kann Defense (4 moves)', '1.e4 c6 2.d4 d5 3.Nf3 Bg4', 'Be2'),
        ('english_opening', 'rnbqkb1r/pppp1ppp/5n2/4p3/2P5/5N2/PP1PPPPP/RNBQKB1R w KQkq - 2 3', 'English Opening (3 moves)', '1.c4 e5 2.Nf3 Nf6', 'Nc3')
    ]
    
    for key, fen, title, moves, next_move in sample_positions:
        try:
            # Create position before the next move
            board = chess.Board(fen)
            
            # Determine board orientation (flip if black to move)
            flip_board = not board.turn
            
            # Make the next move to create the sample position
            move = board.parse_san(next_move)
            board.push(move)
            
            # Get quick analysis using built-in chess engine logic
            legal_moves = list(board.legal_moves)
            if legal_moves:
                ...
                for legal_move in legal_moves[:10]:  # Check first 10 moves for speed
                    score = 0
                    if board.is_capture(legal_move):
                        score += 30
                    if legal_move.to_square in [chess.E4, chess.E5, chess.D4, chess.D5]:
                        score += 10
                    if score > best_score:
                        best_score = score
                ...
                if best_move:
                    analysis_move = board.san(best_move)
                    analysis_explanation = f"Recommended continuation after {next_move}. "
                    if board.is_capture(best_move):
                        analysis_explanation += "This capture improves material balance."
                    else:
                        analysis_explanation += "This move develops pieces and controls key squares."
                else:
                    analysis_move = "Analyzing..."
                    analysis_explanation = "Position analysis in progress."
            else:
                analysis_move = "Game over"
                analysis_explanation = "No legal moves available."
            
            sample_boards[key] = {
                'fen': board.fen(),  # FEN after the move
                'title': title,
                'moves': moves,
                'last_move': next_move,
                'html': board_to_html(board, flipped=flip_board),
                'analysis_move': analysis_move,
            ...
            sample_boards[key] = {
                ...
                'moves': moves,
                'last_move': 'Error',
                'html': f'<div style="color: red;">Error generating board: {e}</div>',
                'analysis_move': 'Error',
                'analysis_explanation': f'Analysis failed: {e}'
            }
            ...
            <head>
                ...
                <script>
                    function loadSampleFEN(fen) {
                        console.log('=== loadSampleFEN Debug ===');
                        console.log('FEN received:', fen);
                        console.log('FEN type:', typeof fen);
                        console.log('FEN length:', fen.length);
                        
                        const fenInput = document.getElementById('fen');
                        console.log('FEN input element found:', !!fenInput);
                        
                        if (fenInput) {
                            console.log('Current input value before:', fenInput.value);
                            fenInput.value = fen;
                            console.log('Input value after setting:', fenInput.value);
                            console.log('Values match:', fenInput.value === fen);
                            
                            // Trigger input event manually to ensure validation runs
                            ...
                            console.log('=== End loadSampleFEN Debug ===');
                        } else {
                            console.error('ERROR: FEN input element with id="fen" not found!');
                    ...
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        {% if fen_result %}
                        setAnalyzedState();
                        hideSampleBoards(); // Hide sample boards when showing results
                        {% else %}
                        showSampleBoards(); // Show sample boards on initial load
                        {% endif %}
                    });
                </script>
                        ...
                        <div style="margin-bottom: 15px;">
                            ...
                            <div style="margin-top: 8px; padding: 6px; background: rgba(142, 68, 173, 0.05); border-radius: 4px; border: 1px solid rgba(142, 68, 173, 0.2);">
                                ...
                                <div style="font-size: 10px; color: #6a5d7a; text-align: center; margin-top: 2px; font-style: italic;">
                                    💡 Tip: Use "Create FEN Position" to set up any chess position, then copy the FEN notation back here
                                                ...
                                                <div class="sample-fens" id="sample_fens_area" style="display: block;">
                            <div style="font-weight: bold; margin-bottom: 15px; color: #4a2c7a; text-align: center; font-size: 16px;">🎯 Sample Opening Positions (Next Move Analysis):</div>
                            ...
                            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 15px; margin-bottom: 20px;">
                                ...
                                <div class="sample-position" style="border: 2px solid #c299ff; border-radius: 8px; padding: 12px; background: rgba(255, 255, 255, 0.9);">
                                    ...
                                    <div style="font-size: 9px; color: #8e44ad; text-align: center; margin-top: 3px; font-weight: bold;">Last: {{ sample_boards.italian_game.last_move }}</div>
                                    <div style="margin-top: 8px; padding: 6px; background: rgba(142, 68, 173, 0.1); border-radius: 4px; border: 1px solid rgba(142, 68, 173, 0.2);">
                                        <div style="font-size: 9px; color: #4a2c7a; font-weight: bold;">Next: {{ sample_boards.italian_game.analysis_move }}</div>
                                        <div style="font-size: 8px; color: #6a5d7a; margin-top: 2px;">{{ sample_boards.italian_game.analysis_explanation }}</div>
                                    </div>
                                </div>
                                ...
                                <div class="sample-position" style="border: 2px solid #c299ff; border-radius: 8px; padding: 12px; background: rgba(255, 255, 255, 0.9);">
                                    ...
                                    <div style="font-size: 9px; color: #8e44ad; text-align: center; margin-top: 3px; font-weight: bold;">Last: {{ sample_boards.french_defense.last_move }}</div>
                                    <div style="margin-top: 8px; padding: 6px; background: rgba(142, 68, 173, 0.1); border-radius: 4px; border: 1px solid rgba(142, 68, 173, 0.2);">
                                        <div style="font-size: 9px; color: #4a2c7a; font-weight: bold;">Next: {{ sample_boards.french_defense.analysis_move }}</div>
                                        <div style="font-size: 8px; color: #6a5d7a; margin-top: 2px;">{{ sample_boards.french_defense.analysis_explanation }}</div>
                                    </div>
                                </div>
                                ...
                                <div class="sample-position" style="border: 2px solid #c299ff; border-radius: 8px; padding: 12px; background: rgba(255, 255, 255, 0.9);">
                                    ...
                                    <div style="font-size: 9px; color: #8e44ad; text-align: center; margin-top: 3px; font-weight: bold;">Last: {{ sample_boards.queens_gambit.last_move }}</div>
                                    <div style="margin-top: 8px; padding: 6px; background: rgba(142, 68, 173, 0.1); border-radius: 4px; border: 1px solid rgba(142, 68, 173, 0.2);">
                                        <div style="font-size: 9px; color: #4a2c7a; font-weight: bold;">Next: {{ sample_boards.queens_gambit.analysis_move }}</div>
                                        <div style="font-size: 8px; color: #6a5d7a; margin-top: 2px;">{{ sample_boards.queens_gambit.analysis_explanation }}</div>
                                    </div>
                                </div>
                                ...
                                <div class="sample-position" style="border: 2px solid #c299ff; border-radius: 8px; padding: 12px; background: rgba(255, 255, 255, 0.9);">
                                    ...
                                    <div style="font-size: 9px; color: #8e44ad; text-align: center; margin-top: 3px; font-weight: bold;">Last: {{ sample_boards.caro_kann.last_move }}</div>
                                    <div style="margin-top: 8px; padding: 6px; background: rgba(142, 68, 173, 0.1); border-radius: 4px; border: 1px solid rgba(142, 68, 173, 0.2);">
                                        <div style="font-size: 9px; color: #4a2c7a; font-weight: bold;">Next: {{ sample_boards.caro_kann.analysis_move }}</div>
                                        <div style="font-size: 8px; color: #6a5d7a; margin-top: 2px;">{{ sample_boards.caro_kann.analysis_explanation }}</div>
                                    </div>
                                </div>
                                ...
                                <div class="sample-position" style="border: 2px solid #c299ff; border-radius: 8px; padding: 12px; background: rgba(255, 255, 255, 0.9);">
                                    ...
                                    <div style="font-size: 9px; color: #8e44ad; text-align: center; margin-top: 3px; font-weight: bold;">Last: {{ sample_boards.english_opening.last_move }}</div>
                                    <div style="margin-top: 8px; padding: 6px; background: rgba(142, 68, 173, 0.1); border-radius: 4px; border: 1px solid rgba(142, 68, 173, 0.2);">
                                        <div style="font-size: 9px; color: #4a2c7a; font-weight: bold;">Next: {{ sample_boards.english_opening.analysis_move }}</div>
                                        <div style="font-size: 8px; color: #6a5d7a; margin-top: 2px;">{{ sample_boards.english_opening.analysis_explanation }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
